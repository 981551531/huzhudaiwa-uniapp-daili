...
// 执行应用重启操作
const performAppReload = function(data, popupObj) {
	try {
		// 3. 数据持久化机制 & 7. 记录更新版本号与时间戳
		uni.setStorageSync('lastUpdateVersion', data.version || data.versionName || 'unknown');
		uni.setStorageSync('lastUpdateTime', Date.now());
		// 触发全局事件，供业务侧保存关键数据
		uni.$emit('app-update-before-reload');

		// 4. 平滑重载过渡，避免闪屏或卡顿
		if (popupObj) {
			popupObj.change({
				contentText: "应用资源更新完成，正在重启...",
				buttonNum: 0,
				progress: false
			});
		} else {
		...
		// 6. 提供可配置的延迟时间（默认500ms），确保系统资源释放完成
		const delay = typeof data.reloadDelay === 'number' ? data.reloadDelay : 500;
		
		setTimeout(() => {
			if (popupObj) popupObj.cancel();
			else uni.hideLoading();
			// 1 & 2. 触发自动重新加载逻辑，确保安装完整后执行
			plus.runtime.restart();
		}, delay);
	} catch (error) {
		// 5. 增加重载失败时的降级处理与错误提示
		console.error("应用自动重启失败：", error);
		if (popupObj) {
			popupObj.change({
				contentText: "自动重启失败，请手动重启应用生效",
				buttonNum: 1,
				progress: false
			});
		} else {
			uni.showModal({
				title: '更新提示',
				content: '应用资源已更新，但自动重启失败，请手动重启应用生效。',
				showCancel: false
			});
		}
	}
		...
		dtask = plus.downloader.createDownload(data.downloadUrl, {
			...
			if (status == 200) {
				...
				plus.runtime.install(download.filename, {}, function() {
					// 替换原本需要用户点击的代码，实现自动重启
					performAppReload(data, popupObj);
				}, function(e) {
					popupObj.cancel();
		...
		dtask = plus.downloader.createDownload(data.downloadUrl, {
			...
			if (status == 200) {
				plus.runtime.install(download.filename, {}, function() {
					...
					// 替换原无操作逻辑，实现静默更新后的自动重启
					performAppReload(data, null);
				}, function(e) {
					plus.nativeUI.alert("安装文件失败[" + e.code + "]：" + e.message);
				});
			} else {
